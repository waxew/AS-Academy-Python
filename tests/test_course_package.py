...
def test_assessment_depth_is_release_ready():
    ...
    assert 0 <= final_exam["passingPercent"] <= 100
    assert len(questions)>=40
    assert len({q["id"] for q in questions})==len(questions)
    assert len({q["category"] for q in questions})>=10
    assert all(len(q["choices"])>=2 and q["answer"] in q["choices"] for q in questions)
...
def test_learning_map_references_are_valid():
    learning_map=load_json(PACKAGE/"learning-map.json")
    levels={x["id"] for x in load_json(PACKAGE/"levels.json")}
    exercises={x["id"] for x in load_json(PACKAGE/"exercises/bank.json")}
    quizzes={x["id"] for x in load_json(PACKAGE/"quizzes/bank.json")}
    projects={x["id"] for x in load_json(PACKAGE/"projects/registry.json")}
    reviews=set(load_json(PACKAGE/"review/spaced-practice.json")["checkpoints"])
    mapped={x["levelId"] for x in learning_map["levels"]}
    assert mapped==levels
    for entry in learning_map["levels"]:
        assert set(entry["exerciseIds"])<=exercises
        assert set(entry["quizIds"])<=quizzes
        assert set(entry["projectIds"])<=projects
        assert entry["reviewCheckpointId"] in reviews
...
def test_release_candidate_declares_required_quality_gates():
    ...
    assert rc["status"] in {"release-candidate","stable"}
